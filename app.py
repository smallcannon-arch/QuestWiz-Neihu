...
# ==========================================
# 1. 增強型檔案處理工具 (支援 PDF, Word, Excel, CSV, PPT)
# ==========================================
def read_pdf(file):
...
### ⚡ 行政配分核心指令：
1. **自動偵測節數**：請掃描教材或上傳的審核表資料，尋找「X節」或「X堂課」關鍵字。
2. **比例配分公式**：子單元配分 = (該單元節數 / 總節數) * 100 分。
3. **高品質命題**：結合 PIRLS、TIMSS、PISA 等國際標準，強調情境化與整合應用。
...
with st.sidebar:
    ...
    st.divider()
    st.info("💡 系統已開啟「自動節數偵測」，AI 將自行計算配分比例。")
...
if not st.session_state.chat_history:
    with st.container(border=True):
        ...
        uploaded_files = st.file_uploader("上傳教材或審核表 (支援 PDF, Word, Excel, CSV, 圖片)", 
                                         type=["pdf", "docx", "doc", "csv", "xlsx", "pptx", "jpg", "png"], 
