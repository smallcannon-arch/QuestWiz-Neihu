...
st.markdown("""
    ...
    .comfort-box { background-color: #1E293B; padding: 20px; border-radius: 12px; border-left: 6px solid #3B82F6; font-size: 16px; color: #CBD5E1; line-height: 2.0; }
...
with st.sidebar:
    ...
    st.markdown("### 🚀 快速指南")
    st.markdown("""<div class="comfort-box">1️⃣ 前往 <a href="https://aistudio.google.com/" target="_blank">AI Studio</a><br>2️⃣ 登入<b>個人帳號</b><br>3️⃣ 點擊 <b>Get API key</b> 並貼入下方</div>""", unsafe_allow_html=True)
    st.write('</div>', unsafe_allow_html=True)
    st.write('<div class="sb-section">', unsafe_allow_html=True)
    st.markdown("### 🔑 金鑰設定")
    api_input = st.text_area("貼上 API Key", height=100)
    if st.button("🔄 重置系統進度"):
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 產出學習目標審核表", type="primary", use_container_width=True):
            if not api_input or not grade or not subject or not uploaded_files:
                st.warning("⚠️ 請確認 API Key、參數與教材均已備妥。")
            else:
                with st.spinner("⚡ 正在極速掃描教材並精準提取配分目標..."):
                    model_name, _ = find_available_model(api_input.strip(), "flash")
                    content = extract_text_from_files(uploaded_files)
                    # 強化版配分任務指令 [cite: 2026-02-13]
                    sys_inst = """你是一位專業教務主任。請嚴格根據教材內容原文產出【學習目標審核表】表格。
                    任務要求：
                    1. 欄位：| 單元名稱 | 學習目標(原文) | 對應題型 | 預計配分 |
                    2. 對應邏輯：每一項學習目標必須單獨列出，並從勾選題型中挑選最適合的 1-3 個題型。
                    3. 配分邏輯：根據目標的重要程度，給予單項目標 2-8 分的配分，確保整張表的對應具備邏輯性。
                    4. 禁止：嚴禁在此階段產出題目！"""
                    try:
...
# --- Phase 2: 確認與出題 ---
    ...
    if df is not None:
        ...
        with c_d1:
            ...
            except: st.caption("優先使用 CSV 匯出。")
        with c_d2:
            st.download_button("📥 下載 CSV 審核表 (保險用)", data=df.to_csv(index=False).encode('utf-8-sig'), file_name=f"內湖國小_{subj_name}_審核表.csv", use_container_width=True)

    st.divider()
    if st.button("✅ 確認無誤，開始出題", type="primary", use_container_width=True):
        with st.spinner("🧠 旗艦大腦 Pro 命題中，將嚴格對應審核表分數..."):
