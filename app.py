...
# --- 3. 核心數據轉換工具 (含 CSV 保險) --- [cite: 2026-02-13]
def process_table_data(md_text):
    """將 Markdown 轉為 DataFrame，若失敗回傳 None"""
    try:
        cleaned_text = md_text.replace("｜", "|").replace("**", "").replace("||", "|\n|")
        ...
        raw_cells = [c.strip() for c in cleaned_text[start_index:].split('|') if c.strip() and '---' not in c]
        
        num_cols = 4 
        if len(raw_cells) < num_cols: return None, None
...
st.markdown("""
    ...
    .school-header {
        ...
        padding: 25px; border-radius: 18px; text-align: center; margin-bottom: 25px; border: 1px solid #475569;
    }
    ...
    .comfort-box {
        ...
        margin-bottom: 12px; border-left: 5px solid #3B82F6; font-size: 13px; color: #CBD5E1; line-height: 1.6;
    }
    .comfort-box a { color: #60A5FA !important; text-decoration: none; font-weight: bold; }
    
    [data-testid="stSidebar"] .stButton > button { display: block; margin: 10px auto !important; width: 100%; height: 40px; }
    .custom-footer { position: fixed; left: 0; bottom: 0; width: 100%; background-color: #0F172A; color: #475569; text-align: center; padding: 10px; font-size: 11px; border-top: 1px solid #1E293B; z-index: 100; }
    </style>
...
with st.sidebar:
    ...
    api_input = st.text_area("在此輸入 API Key", height=70, placeholder="請貼上金鑰...")
    if st.button("🔄 重置系統"):
        ...
        st.rerun()
    st.markdown("### 📚 資源連結")
    st.markdown("""<div class="comfort-box"><b>教材下載：</b><a href="https://webetextbook.knsh.com.tw/" target="_blank">康軒</a> | <a href="https://edisc3.hle.com.tw/" target="_blank">翰林</a> | <a href="https://reader.nani.com.tw/" target="_blank">南一</a><br><b>參考資料：</b><a href="https://cirn.moe.edu.tw/Syllabus/index.aspx?sid=1108" target="_blank">108課綱</a> | <a href="https://www.nhps.hc.edu.tw/" target="_blank">校網</a></div>""", unsafe_allow_html=True)

# --- Phase 1: 參數設定與教材上傳 --- [cite: 2026-02-13]
if st.session_state.phase == 1:
    with st.container(border=True):
        st.markdown("### 📍 第一階段：參數設定與教材上傳")
        c1, c2, c3 = st.columns(3)
        with c1: grade = st.selectbox("1. 年級", ["", "一年級", "二年級", "三年級", "四年級", "五年級", "六年級"], index=0)
        with c2: subject = st.selectbox("2. 科目", ["", "國語", "數學", "自然科學", "社會", "英語"], index=0)
        with c3: mode = st.selectbox("3. 模式", ["🟢 模式 A：適中", "🔴 模式 B：困難", "🌟 模式 C：素養"], index=0)
        ...
        if st.button("🚀 產出學習目標審核表", type="primary", use_container_width=True):
            if not api_input or not grade or not subject or not uploaded_files:
                ...
                with st.spinner("⚡ 正在分析教材並原文提取學習目標..."):
                    genai.configure(api_key=random.choice([k.strip() for k in api_input.replace('\n', ',').split(',') if k.strip()]))
                    content = extract_text_from_files(uploaded_files)
                    try:
                        model = genai.GenerativeModel("gemini-1.5-flash", system_instruction="你僅產出【學習目標審核表】表格。欄位：| 單元名稱 | 學習目標(原文) | 對應題型 | 預計配分 |。嚴禁產出試題！")
...
# --- Phase 2: 確認與出題 ---
    ...
    # 嘗試轉換數據 [cite: 2026-02-13]
    df, error = process_table_data(current_md)
    
    col_d1, col_d2 = st.columns(2)
    if df is not None:
        with col_d1:
            # 嘗試 Excel [cite: 2026-02-13]
            try:
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    df.to_excel(writer, index=False)
                st.download_button("📥 下載 Excel 審核表", data=buffer.getvalue(), file_name=f"內湖國小_{subject}_審核表.xlsx", use_container_width=True)
            except:
                st.caption("Excel 組件尚未就緒，請改用 CSV。")
        with col_d2:
            # 萬能保險：CSV [cite: 2026-02-13]
            csv_data = df.to_csv(index=False).encode('utf-8-sig')
            st.download_button("📥 下載 CSV 審核表 (保險用)", data=csv_data, file_name=f"內湖國小_{subject}_審核表.csv", use_container_width=True)
    else:
        st.error(f"⚠️ 表格解析異常。")
        with st.expander("🔍 原始資料"): st.text(current_md)
