...
# 解決 ResourceExhausted 的關鍵函式 [cite: 2026-02-13]
def generate_with_retry(model_or_chat, prompt, stream=True):
    ...
    for i in range(max_retries):
        try:
            ...
            err_msg = str(e).lower()
            if "429" in err_msg or "exhausted" in err_msg: # 同時捕捉 ResourceExhausted [cite: 2026-02-13]
                wait = (i + 1) * 10 # 拉長重試等待：10秒, 20秒...
                st.toast(f"⏳ 系統忙碌(配額限額)，{wait}秒後自動重試...", icon="⚠️")
                time.sleep(wait)
            else: raise e
    raise Exception("API 額度過載，請稍後幾分鐘再試。")
...
st.markdown("""
    ...
    .comfort-box { background-color: #1E293B; padding: 20px; border-radius: 12px; border-left: 6px solid #3B82F6; font-size: 16px; color: #CBD5E1; line-height: 2.2; }
...
with st.sidebar:
    ...
    st.markdown("### 🚀 一、取得金鑰")
    st.markdown("""<div class="comfort-box">1️⃣ 前往 <a href="https://aistudio.google.com/" target="_blank">AI Studio (點我)</a><br>2️⃣ 登入<b>個人帳號</b><br>3️⃣ 點擊下方 <b>Get API key</b> 並貼入下方</div>""", unsafe_allow_html=True)
    ...
    st.write('<div class="sb-section">', unsafe_allow_html=True)
    st.markdown("### 📚 資源連結")
    st.markdown("""<div class="comfort-box"><b>📖 教材：</b><a href="https://webetextbook.knsh.com.tw/" target="_blank">康軒</a> | <a href="https://edisc3.hle.com.tw/" target="_blank">翰林</a> | <a href="https://reader.nani.com.tw/" target="_blank">南一</a><br><br><b>🏛️ 官方：</b><a href="https://cirn.moe.edu.tw/Syllabus/index.aspx?sid=1108" target="_blank">108 課綱</a> | <a href="https://www.nhps.hc.edu.tw/" target="_blank">內湖校網</a></div>""", unsafe_allow_html=True)
    st.write('</div>', unsafe_allow_html=True)
...
if st.session_state.phase == 1:
    with st.container(border=True):
        ...
        uploaded_files = st.file_uploader("5. 上傳教材", type=["pdf", "docx", "doc"], accept_multiple_files=True)
    ...
    if df is not None:
        ...
        with c_d1:
            try:
                ...
                st.download_button("📥 下載 Excel", data=buf.getvalue(), file_name=f"內湖國小_{subj_name}.xlsx", use_container_width=True)
            except: st.caption("優先使用 CSV。")
        with c_d2:
            st.download_button("📥 下載 CSV", data=df.to_csv(index=False).encode('utf-8-sig'), file_name=f"內湖國小_{subj_name}.csv", use_container_width=True)

    st.divider()
    if st.button("✅ 確認無誤，開始出題", type="primary", use_container_width=True):
        with st.spinner("🧠 正在換檔至 Pro 模型深度命題 (若配額滿載將自動排隊)..."):
            model_name_pro, _ = find_available_model(api_input.strip(), "pro")
            model_pro = genai.GenerativeModel(model_name_pro)
            # 重試機制保護 Phase 2 [cite: 2026-02-13]
            try:
